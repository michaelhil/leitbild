...
import { createMicroworldWorkspaceRuntimeRegistry, type MicroworldWorkspaceRuntimeRegistry } from './runtime-registry.ts'
...
const createRegistry = async () => {
  const dataDir = await mkdtemp(join(tmpdir(), 'microworld-workspace-runtime-'))
...
describe('Microworld Workspace runtime registry', () => {
  test('accepts only a Host-provisioned id and deduplicates concurrent loads', async () => {
    ...
    await expect(registry.getOrLoad(newWorkspaceId())).rejects.toThrow('Microworld Module not provisioned')
  })
  ...
  test('distinguishes unloading from destructive Module removal', async () => {
    ...
    expect(registry.getOrLoad(workspaceId)).rejects.toThrow('Microworld Module not provisioned')
    expect(Bun.file(join(microworldWorkspacePaths(dataDir, workspaceId).simulationRuns, simulationRun.id, 'manifest.json')).exists()).resolves.toBe(false)
  })
})
