import type { ControlInstanceId, IsoTimestamp } from '../model/index.ts'

export interface ControlInstanceRuntimeMetricsSnapshot {
...
export interface ControlInstanceRuntimeMetricsRecorder {
  ...
  readonly snapshot: () => ControlInstanceRuntimeMetricsSnapshot
}

const roundMetricMs = (value: number): number =>
  Math.round(value * 10) / 10

export const createControlInstanceRuntimeMetricsRecorder = (config: {
  readonly controlInstanceId: ControlInstanceId
  readonly createdAt: IsoTimestamp
}): ControlInstanceRuntimeMetricsRecorder => {
  ...
  return {
    ...
    snapshot: (): ControlInstanceRuntimeMetricsSnapshot => ({
