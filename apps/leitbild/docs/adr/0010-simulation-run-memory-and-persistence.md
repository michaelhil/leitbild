# ADR 0010: Simulation Run Memory and Persistence Lifecycle

## Decision

Leitbild will separate Simulation Run state into three runtime/persistence layers:

- **Projected State**: the current canonical operational picture for a Simulation Run.
...
All accepted updates still flow through the Simulation Run runtime. The runtime applies valid updates to Projected State and broadcasts them to the Live Change Feed. The runtime persists only durable events to the Durable Journal.
...
Simulation providers keep **Provider Private State** and **Provider Projections** needed for their mechanics. They do not own the canonical shared operational picture and do not retain Simulation Run history. Providers may observe committed Domain Events to update their private mechanics or projections.

## Rationale

The first sandbox deployment showed that treating every `object.upserted` event as durable history does not scale. A long-running Simulation Run accumulated a large JSONL event log from per-second movement updates, and startup/rejoin paths had to read and retain too much history.
...
- Event persistence classification is part of the Simulation Run runtime, not simulation providers, so multi-provider memory policy stays consistent.
