...
import { roomSelectionIncludingRun, roomSelectsRun, sameResource } from '../src/ui/run-room-scope.ts'
...
describe('Run Room scope', () => {
  test('compares complete Resource identity when family and original share an id', () => {
    ...
    expect(roomSelectsRun(room([{ rel: 'subject-collection', ref: familyRef }]), run)).toBe(true)
  })

  test('supports dynamic all-except and fixed selected-only links', () => {
    expect(roomSelectsRun(room([
      { rel: 'subject-collection', ref: familyRef },
      { rel: 'subject-excluded', ref: runRef },
    ]), run)).toBe(false)
    expect(roomSelectsRun(room([
      { rel: 'subject-collection', ref: familyRef },
      { rel: 'subject-member', ref: runRef },
    ]), run)).toBe(true)
  })
  ...
  test('explicit reopening restores one excluded Run without changing future-copy policy', () => {
    ...
    const dynamicRoom = room([
      { rel: 'subject-collection', ref: familyRef },
      { rel: 'subject-excluded', ref: run.ref },
      { rel: 'subject-excluded', ref: other },
    ])
    expect(roomSelectionIncludingRun(dynamicRoom, familyRef, run.ref)).toEqual({
      kind: 'collection',
    ...
    const fixedRoom = room([
      { rel: 'subject-collection', ref: familyRef },
      { rel: 'subject-member', ref: other },
    ])
    expect(roomSelectionIncludingRun(fixedRoom, familyRef, run.ref)).toEqual({
      kind: 'collection',
      collection: familyRef,
