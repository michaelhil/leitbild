...
describe('createWikiSource', () => {
  ...
  test('builds citation + raw URLs from the binding', () => {
    ...
    expect(src.citationUrl('E-0')).toBe('https://leitbild-wikis.github.io/pwr-ops/procedures/E-0/')
    expect(src.rawUrl('E-0')).toBe('https://raw.githubusercontent.com/leitbild-wikis/pwr-ops/main/wiki/procedures/E-0.md')
  })
  ...
  test('fetchProcedure hits raw.githubusercontent with the right path', async () => {
    ...
    expect(calls[0]).toBe('https://raw.githubusercontent.com/leitbild-wikis/pwr-ops/main/wiki/procedures/E-0.md')
  })
  ...
  test('HTTP error throws with id + status', async () => {
    ...
    await expect(src.fetchProcedure('XYZ-99')).rejects.toThrow(/leitbild-wikis\/pwr-ops/)
  })
})
...
describe('wiki-fetcher — GitHub Pages fallback for manifest', () => {
  ...
  test('procedure markdown falls back to Pages sidecar when raw fails', async () => {
    ...
    globalThis.fetch = ((input: string | URL | Request) => {
      ...
      if (url === 'https://leitbild-wikis.github.io/pwr-ops/procedures/E-0.md') {
        sidecarCalls += 1
