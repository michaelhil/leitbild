...
import { leitbildHome } from '../core/paths.ts'
...
export const defaultLogDir = (): string => join(leitbildHome(), 'logs')
...
export const parseLogConfigFromEnv = (env: NodeJS.ProcessEnv = process.env): LogConfig => {
  const enabled = env.LEITBILD_LOG_ENABLED === '1'
  const dir = env.LEITBILD_LOG_DIR ?? defaultLogDir()
  const sessionId = env.LEITBILD_SESSION_ID ?? defaultSessionId()
  const kinds = env.LEITBILD_LOG_KINDS
    ? env.LEITBILD_LOG_KINDS.split(',').map(s => s.trim()).filter(Boolean)
    : ['*']
