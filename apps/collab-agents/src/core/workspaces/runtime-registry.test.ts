...
import { createCollabAgentsModuleState, type CollabAgentsModuleState } from './module-state.ts'
...
describe('WorkspaceRuntimeRegistry', () => {
  ...
  let moduleState: CollabAgentsModuleState

  const provision = async (id: WorkspaceId): Promise<void> => {
    await moduleState.provision(id, 'collab')
  ...
  beforeEach(async () => {
    ...
    moduleState = createCollabAgentsModuleState()
  ...
  it('seeds only when Collab and Agents are composed', async () => {
    const id = newWorkspaceId()
    await provision(id)
    delete process.env.LEITBILD_SEED_WORKSPACE
  ...
  it('evicts then lazy-reloads with state preserved', async () => {
    ...
    const stats = await stat(workspaceModulePaths(id).collab.snapshot)
  ...
  it('rejects loading an unprovisioned Workspace', async () => {
    await expect(registry.getOrLoad(newWorkspaceId())).rejects.toThrow('no provisioned Leitbild Modules')
  })
  ...
  it('shutdown flushes every active Workspace', async () => {
    ...
    await registry.shutdown()
    expect(registry.list()).toEqual([])
    await stat(workspaceModulePaths(idA).collab.snapshot)
    await stat(workspaceModulePaths(idB).collab.snapshot)
  })
