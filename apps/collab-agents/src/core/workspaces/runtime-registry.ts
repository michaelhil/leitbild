...
//
// One process holds many Workspaces; each is a CollabAgentsWorkspaceRuntime composed from
// its enabled Collab and Agents Module shards. The registry lazy-loads from disk,
// keeps the CollabAgentsWorkspaceRuntime in memory while active, and evicts idle ones after
...
//   1. createCollabAgentsWorkspaceRuntime(...)           — new in-memory CollabAgentsWorkspaceRuntime, no events wired.
...
import { createCollabAgentsWorkspaceRuntime } from '../../main.ts'
import {
...
import type { CollabAgentsModuleState } from './module-state.ts'
...
export interface WorkspaceRuntimeRegistryOptions {
  ...
  readonly moduleState: CollabAgentsModuleState
  ...
  // Hook called immediately after a fresh CollabAgentsWorkspaceRuntime is constructed (either
  ...
  readonly onWorkspaceRuntimeCreated?: (runtime: CollabAgentsWorkspaceRuntime, id: WorkspaceId, autoSaver: ModuleAutoSaver) => Promise<void> | void
  // Hook called immediately before a CollabAgentsWorkspaceRuntime is dropped from memory.
  // Bootstrap removes the WS callback wiring here.
  readonly onWorkspaceRuntimeEvicted?: (runtime: CollabAgentsWorkspaceRuntime, id: WorkspaceId) => void
  ...
  // check still runs before any traffic actually reaches a CollabAgentsWorkspaceRuntime, but
  ...
  readonly onFirstLoad?: (runtime: CollabAgentsWorkspaceRuntime, id: WorkspaceId) => void
}

export interface WorkspaceRuntimeRegistry {
  readonly getOrLoad: (id: WorkspaceId) => Promise<CollabAgentsWorkspaceRuntime>
  ...
  // In-memory only lookup. Returns the live CollabAgentsWorkspaceRuntime for `id` if it is
  ...
  readonly tryGetLive: (id: WorkspaceId) => CollabAgentsWorkspaceRuntime | undefined
...
export const createWorkspaceRuntimeRegistry = (opts: WorkspaceRuntimeRegistryOptions): WorkspaceRuntimeRegistry => {
  ...
  const pendingLoads = new Map<WorkspaceId, Promise<CollabAgentsWorkspaceRuntime>>()
  ...
  const drainAgents = async (system: CollabAgentsWorkspaceRuntime): Promise<void> => {
  ...
  // Build a fresh CollabAgentsWorkspaceRuntime for `id`, restoring from snapshot if present.
  ...
  const buildWorkspaceRuntime = async (id: WorkspaceId): Promise<{ system: CollabAgentsWorkspaceRuntime; autoSaver: ModuleAutoSaver }> => {
    const paths = workspaceModulePaths(id)
    const enabledModules = await opts.moduleState.enabled(id)
    if (enabledModules.size === 0) throw new Error(`Workspace has no provisioned Leitbild Modules: ${id}`)
    const system = createCollabAgentsWorkspaceRuntime({
    ...
    const shouldSeed = enabledModules.has('collab')
      && enabledModules.has('agents')
      && snapshots.collab === null
  ...
  const fireFirstLoadOnce = (system: CollabAgentsWorkspaceRuntime, id: WorkspaceId): void => {
  ...
  const getOrLoad = async (id: WorkspaceId): Promise<CollabAgentsWorkspaceRuntime> => {
    ...
    const loadPromise = (async (): Promise<CollabAgentsWorkspaceRuntime> => {
      try {
  ...
  const evictOne = async (id: WorkspaceId): Promise<void> => {
    ...
    const evictionPromise = (async (): Promise<void> => {
      ...
      // timer closures pin the entire CollabAgentsWorkspaceRuntime (RoomDirectory + agents + history) and
      ...
      // intervals (no setInterval pinning the CollabAgentsWorkspaceRuntime), but in-flight LLM
  ...
  const listOnDisk = async (): Promise<ReadonlyArray<WorkspaceModuleOnDisk>> => {
    ...
    for (const name of entries) {
      ...
      for (const path of [paths.collab.snapshot, paths.agents.snapshot]) {
        try {
  ...
  const tryGetLive = (id: WorkspaceId): CollabAgentsWorkspaceRuntime | undefined => {
    const entry = map.get(id)
