// ============================================================================
// leitbild — CollabAgentsWorkspaceRuntime Factory + Entry Point
//
// createCollabAgentsWorkspaceRuntime() builds the full system. Can be imported without side effects.
...
export interface CollabAgentsWorkspaceRuntime {
  ...
  // Same Workspace across every CollabAgentsWorkspaceRuntime in this process; surfaced via
...
export interface CreateCollabAgentsWorkspaceRuntimeOptions {
  // Pre-built deployment runtime. When passed, createCollabAgentsWorkspaceRuntime skips internal
  ...
  // Per-Workspace vector store path (RAG features). When set, createCollabAgentsWorkspaceRuntime
...
export const createCollabAgentsWorkspaceRuntime = (options: CreateCollabAgentsWorkspaceRuntimeOptions = {}): CollabAgentsWorkspaceRuntime => {
  ...
  // CollabAgentsWorkspaceRuntime default chain comes from deployment.llmPolicyStore (loaded by
  // bootstrap from ~/.leitbild/llm-policy.json); read at request time so
  ...
  // proposing an event-bus replacement or createCollabAgentsWorkspaceRuntime split.
  ...
  // CollabAgentsWorkspaceRuntime-level membership operations — extracted to core/room-operations.ts.
  ...
  // Forward-ref so the runner can call CollabAgentsWorkspaceRuntime.* without a build-order cycle.
  const systemRef: { current: CollabAgentsWorkspaceRuntime | undefined } = { current: undefined }
  const scriptRunner = createScriptRunner({
    getRuntime: () => systemRef.current as CollabAgentsWorkspaceRuntime,
  ...
  // (multi-Workspace) — but when this CollabAgentsWorkspaceRuntime is built directly in tests,
  // we set the dispatcher to forward
  // events to *this* CollabAgentsWorkspaceRuntime's late-bound subscribers. Multi-Workspace
  ...
  // *this* CollabAgentsWorkspaceRuntime. Workspace boot replaces this dispatcher via
  ...
  const system: CollabAgentsWorkspaceRuntime = {
