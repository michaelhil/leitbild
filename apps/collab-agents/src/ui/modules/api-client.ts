...
export const currentWorkspaceId = (): string => {
  if (typeof location === 'undefined') throw new Error('Leitbild Workspace URL is unavailable')
  const match = location.pathname.match(/^\/workspaces\/([^/]+)\/(?:collab|agents)$/)
  const value = match ? decodeURIComponent(match[1] ?? '') : ''
  if (!workspaceIdPattern.test(value)) {
    throw new Error('Leitbild Workspace URL is missing or invalid')
  }
  return value
}

const currentModuleId = (): 'collab' | 'agents' => {
  const match = location.pathname.match(/^\/workspaces\/[^/]+\/(collab|agents)$/)
  if (!match) throw new Error('Leitbild Module URL is missing or invalid')
  return match[1] as 'collab' | 'agents'
}
...
export const apiPath = (path: string): string => {
  ...
  return isDeploymentPath(path)
    ? `/api${path}`
    : `/api/workspaces/${currentWorkspaceId()}/${currentModuleId()}${path}`
}
...
export const workspaceRealtimeUrl = (sessionToken: string): string => {
  ...
  return `${protocol}//${location.host}/api/workspaces/${currentWorkspaceId()}/${currentModuleId()}/ws${suffix}`
}
