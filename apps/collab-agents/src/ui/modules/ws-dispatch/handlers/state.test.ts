...
describe('stateHandlers.snapshot — stale cache eviction', () => {
  ...
  beforeEach(() => {
    ...
    Object.defineProperty(globalThis, 'document', {
      configurable: true,
      value: { cookie: `leitbild_workspace=${workspaceId}` },
      writable: true,
    })
    Object.defineProperty(globalThis, 'location', {
      configurable: true,
      value: { pathname: `/workspaces/${workspaceId}/collab` },
      writable: true,
    })
  ...
  test('triggers fetchRoomMessages for the selected room', async () => {
    ...
    expect(fetchCalls.some(u => u.includes(`/api/workspaces/${workspaceId}/collab/rooms/R1`))).toBe(true)
  ...
  test('no fetch when no room is selected', () => {
    ...
    expect(fetchCalls.filter(u => u.includes(`/api/workspaces/${workspaceId}/collab/rooms/`))).toEqual([])
