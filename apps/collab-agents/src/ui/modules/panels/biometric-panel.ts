...
//   - "Stop all active captures" — broadcasts leitbild:biometric-stop-all
//   - In-memory revocation log (last 20 captures, cleared on reload)
//
// The panel mounts only while the leitbild-biometrics pack is installed
...
const renderPanel = async (host: HTMLElement): Promise<void> => {
  ...
  stopAllBtn.addEventListener('click', () => {
    document.dispatchEvent(new CustomEvent('leitbild:biometric-stop-all', { detail: { reason: 'panel-stop-all' } }))
  })
}
