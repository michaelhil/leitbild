...
// 30 min), and persisted in independent Collab and Agents Module shards.
...
import { appendCollabPendingScrub } from './core/storage/module-snapshots.ts'
...
import { createCollabAgentsModuleState } from './core/workspaces/module-state.ts'
...
export const bootstrap = async (): Promise<void> => {
  ...
  const moduleState = createCollabAgentsModuleState()
  ...
  // Systems wrap this in an overlay (see createCollabAgentsWorkspaceRuntime in main.ts).
  ...
  // createCollabAgentsWorkspaceRuntime and live in the per-Workspace overlay.
  ...
  const registry = createWorkspaceRuntimeRegistry({
    ...
    onWorkspaceRuntimeCreated: async (system, id, autoSaver) => {
      ...
      // that adds Workspace-bound built-ins on top. See main.ts createCollabAgentsWorkspaceRuntime.
  ...
  if (packsEnabled) {
    ...
    const crossWorkspaceScrubActivePacks = async (
      ...
      for (const meta of registry.list()) {
        ...
        const snapshotPath = workspaceModulePaths(meta.id).collab.snapshot
        scrubPromises.push(
          appendCollabPendingScrub(snapshotPath, { packId: packId, scheduledAt })
            .then(result => {
  ...
  if (headless) {
    ...
    await moduleState.provision(headlessId, 'collab')
