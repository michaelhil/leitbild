...
const dispatch = (ws: FakeWS, session: ClientSession, system: CollabAgentsWorkspaceRuntime, wsManager: WSManager, payload: unknown) =>
...
describe('WSManager.safeSend backpressure', () => {
  ...
  test('sweepStaleSessions drops sessions with closed WS + old lastActivity, removes agent', async () => {
    ...
    const fakeSystem = { removeAgent: (id: string) => { removed.push(id); return true } } as unknown as CollabAgentsWorkspaceRuntime
    const wsManager = createWSManager({
