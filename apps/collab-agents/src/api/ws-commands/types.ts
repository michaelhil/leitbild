...
export const requireRoom = (wsManager: WSManager, ws: WSConnection, system: CollabAgentsWorkspaceRuntime, roomName: string): ReturnType<typeof system.rooms.getRoom> => {
  const room = system.rooms.getRoom(roomName)
  if (!room) sendError(wsManager, ws, `Room "${roomName}" not found`)
  return room
}

export const requireAgent = (wsManager: WSManager, ws: WSConnection, system: CollabAgentsWorkspaceRuntime, agentName: string): ReturnType<typeof system.team.getAgent> => {
