...
import {
  collabAgentsModuleIdSchema,
  type CollabAgentsModuleId,
  type CollabAgentsModuleState,
} from '../core/workspaces/module-state.ts'
import { asAIAgent } from '../agents/shared.ts'

const manifestFor = (moduleId: CollabAgentsModuleId) => workspaceModuleManifestSchema.parse({
  module: moduleId === 'collab'
    ? {
        id: moduleId,
        title: 'Collab',
  ...
  ui: {
    workspace: `/workspaces/{workspaceId}/${moduleId}`,
  },
})

export const collabModuleManifest = manifestFor('collab')
export const agentsModuleManifest = manifestFor('agents')

const collabCapabilities: ReadonlyArray<ModuleCapabilityDescriptor> = moduleCapabilityCollectionSchema.parse({
  capabilities: [
    {
      id: 'collab.room.create',
      moduleId: 'collab',
      kind: 'command',
      scope: { kind: 'workspace' },
      title: 'Create Room',
      description: 'Creates a durable collab Room in the Workspace.',
      risk: 'write',
      idempotent: false,
    ...
    },
    {
      ...
      moduleId: 'collab',
      kind: 'query',
      scope: { kind: 'resource', resourceType: 'collab.room' },
      title: 'Read Room',
    ...
    {
      id: 'collab.room.post-message',
      moduleId: 'collab',
      kind: 'command',
      scope: { kind: 'resource', resourceType: 'collab.room' },
      title: 'Post Room Message',
      description: 'Posts an attributed message to the selected collab Room.',
      risk: 'write',
      idempotent: false,
...
const resourcesFor = async (
  ...
  if (moduleId === 'collab') {
    return moduleResourceCollectionSchema.parse({ resources: runtime.rooms.listAllRooms().map(room => ({
      ref: { workspaceId, moduleId, type: 'collab.room', id: room.id },
      title: room.name,
      ...(room.roomPrompt === undefined ? {} : { description: room.roomPrompt }),
      capabilityIds: ['collab.room.read', 'collab.room.post-message'],
      observedAt,
    })) }).resources
  }
...
const invoke = async (
  ...
  if (capabilityId === 'collab.room.create') {
  ...
  if (capabilityId === 'collab.room.read') {
    const roomId = requireResourceId(invocation, 'collab', 'collab.room')
  ...
  if (capabilityId === 'collab.room.post-message') {
    const roomId = requireResourceId(invocation, 'collab', 'collab.room')
    const room = runtime.rooms.getRoom(roomId)
...
export const handleCollabAgentsModuleApi = async (
  request: Request,
  url: URL,
  config: {
    readonly state: CollabAgentsModuleState
    readonly registry: WorkspaceRuntimeRegistry
  },
): Promise<Response | null> => {
  try {
    const manifestMatch = url.pathname.match(/^\/\.well-known\/workspace-module\/(collab|agents)$/)
    if (manifestMatch && request.method === 'GET') {
      const moduleId = collabAgentsModuleIdSchema.parse(manifestMatch[1])
      return json(moduleId === 'collab' ? collabModuleManifest : agentsModuleManifest)
    }

    const match = url.pathname.match(/^\/internal\/(collab|agents)\/workspaces\/([^/]+)(?:\/(resources|capabilities)|\/capabilities\/([^/]+)\/invoke)?$/)
    if (!match) return null
    const moduleId = collabAgentsModuleIdSchema.parse(match[1])
    ...
    if (collection === 'capabilities' && request.method === 'GET') {
      return json(moduleCapabilityCollectionSchema.parse({
        capabilities: moduleId === 'collab' ? collabCapabilities : agentsCapabilities,
      }))
    }
