...
export interface WSData {
  ...
  // before loading or materializing any CollabAgentsWorkspaceRuntime.
...
// Resolver: given an workspaceId, return the live CollabAgentsWorkspaceRuntime if currently in
...
export interface WSManagerDeps {
  readonly getRuntime: (workspaceId: WorkspaceId) => CollabAgentsWorkspaceRuntime | undefined
...
export const createWSManager = (deps: WSManagerDeps): WSManager => {
  ...
  // CollabAgentsWorkspaceRuntime callback wiring (room/membership/agent-activity/provider-events/
  ...
  // it runs at the right time (after the CollabAgentsWorkspaceRuntime is fully populated by
...
export const handleWSMessage = async (
  ...
  raw: string,
  system: CollabAgentsWorkspaceRuntime,
  wsManager: WSManager,
): Promise<void> => {
  let msg: WSInbound
  try {
