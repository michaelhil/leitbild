...
const makeSystem = (): CollabAgentsWorkspaceRuntime => {
  ...
  return {
    ...
    llm: { models: async () => [], chat: async () => ({ content: '', generationMs: 0, tokensUsed: { prompt: 0, completion: 0 } }) } as unknown as CollabAgentsWorkspaceRuntime['llm'],
    ollama,
    providerConfig: { order: ['ollama'], ollamaUrl: 'http://localhost:11434', ollamaMaxConcurrent: 2, cloud: {}, ollamaOnly: false, forceFailProvider: null, droppedFromOrder: [], orderFromUser: false } as unknown as CollabAgentsWorkspaceRuntime['providerConfig'],
    routeMessage: () => [],
...
const call = (system: CollabAgentsWorkspaceRuntime, r: Request, path: string, opts: { remoteAddress?: string } = {}) =>
...
describe('HTTP Routes', () => {
  let system: CollabAgentsWorkspaceRuntime
...
describe('unscoped bootstrap routes', () => {
  ...
  test('diagnostics is served without a per-Workspace CollabAgentsWorkspaceRuntime', async () => {
...
describe('GET /system/limits (no auth)', () => {
  ...
  // --- POST /system/evict ---
  // URL-scoped evict: drops the CollabAgentsWorkspaceRuntime from memory; Module snapshots stay.
