...
import type { CollabAgentsModuleState } from '../core/workspaces/module-state.ts'
...
import { handleCollabAgentsModuleApi } from './workspace-module-api.ts'
...
const workspaceIdFromPagePath = (pathname: string): WorkspaceId | null => {
  const match = pathname.match(/^\/workspaces\/([^/]+)\/(?:collab|agents)$/)
...
export const createServer = (config: ServerConfig) => {
  ...
  const server = Bun.serve<WSData>({
    ...
    async fetch(request, bunServer) {
      ...
      const moduleResponse = await handleCollabAgentsModuleApi(request, url, {
