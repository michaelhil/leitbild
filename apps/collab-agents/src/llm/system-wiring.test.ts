// ============================================================================
// E2E integration test: verifies createCollabAgentsWorkspaceRuntime wires router routing events
// through to the late-bound onProviderBound / onProviderAllFailed / onProvider
// StreamFailed callbacks exposed on CollabAgentsWorkspaceRuntime. Without this wiring, Phase 3
...
describe('CollabAgentsWorkspaceRuntime.llm wiring — router events reach late-bound callbacks', () => {
  test('successful chat fires onProviderBound with agent context', async () => {
    const a = makeGateway()
    const b = makeGateway()
    const setup = makeSetup({ a, b }, ['a', 'b'])
    const system = createCollabAgentsWorkspaceRuntime({ providerConfig: baseConfig, providerSetup: setup })
  ...
  test('callSystemLLM path emits events with agentId=null', async () => {
    ...
    const system = createCollabAgentsWorkspaceRuntime({ providerConfig: { ...baseConfig, order: ['a'] }, providerSetup: setup })
