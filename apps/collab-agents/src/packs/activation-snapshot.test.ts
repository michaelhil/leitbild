...
describe('evicted Collab Pack scrub round-trip', () => {
  let temporaryRoot = ''
  ...
  test('drains queued Pack removals without touching Agent state', async () => {
    ...
    temporaryRoot = await mkdtemp(join(tmpdir(), 'collab-scrub-'))
    ...
    const queued = await appendCollabPendingScrub(paths.collab.snapshot, {
      packId: 'aviation',
      scheduledAt: '2026-08-29T18:00:00.000Z',
    })
    expect(queued.applied).toBe(true)

    const snapshots = await loadWorkspaceModuleSnapshots(paths, new Set(['collab']))
    ...
    expect(next.collab.pendingScrubs).toBeUndefined()
  })
})
