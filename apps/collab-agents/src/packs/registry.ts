...
//
// Sources are configured via LEITBILD_PACK_SOURCES (comma-separated). Each
// entry is one of:
//   - `<owner>`         — list `<owner>/leitbild-pack-*` repos
//   - `<owner>/<repo>`  — a single specific pack repo
//
// Default: `leitbild-packs` (the canonical org once it exists).
//
// The exposed `name` is the canonical install namespace — the repo basename
// with any `leitbild-pack-` prefix stripped. This matches what install_pack
...
// LEITBILD_GH_TOKEN to lift to 5000/hr (the same token bug-reporting uses).
// Results are cached for 5 min so the UI Browse view doesn't hammer the API.
// ============================================================================

const PREFIX = 'leitbild-pack-'
...
//
// Order:
//   1. LEITBILD_PACK_REGISTRY_TOKEN — explicit, intended for this purpose
//   2. unauthenticated — 60 req/hr per IP, fine for a 5-min-cached registry
//
// LEITBILD_GH_TOKEN is intentionally NOT used here — see commit log for
// the symptom (org listing 403'd because the fine-grained scope denied
// every endpoint outside michaelhil/leitbild).
const ghHeaders = (): Record<string, string> => {
  const h: Record<string, string> = {
    'Accept': 'application/vnd.github+json',
    'X-GitHub-Api-Version': '2022-11-28',
    'User-Agent': 'leitbild-pack-registry',
  }
  const token = process.env.LEITBILD_PACK_REGISTRY_TOKEN
...
// convention — `leitbild-packs`, `acme-packs`, etc. Operators putting their
// personal account in LEITBILD_PACK_SOURCES still get the prefix filter so
// random repos don't pollute the registry.
const isPackOnlyOwner = (owner: string): boolean =>
  /-packs$/.test(owner) || owner === 'leitbild-packs'
...
export const getAvailablePacks = async (): Promise<ReadonlyArray<RegistryPack>> => {
  ...
  const sources = parseSources(process.env.LEITBILD_PACK_SOURCES)
