...
beforeEach(() => {
  prevHome = process.env.LEITBILD_HOME
  testDir = mkdtempSync(join(tmpdir(), 'leitbild-geo-resolver-test-'))
  process.env.LEITBILD_HOME = testDir
})

afterEach(() => {
  if (prevHome === undefined) delete process.env.LEITBILD_HOME
  else process.env.LEITBILD_HOME = prevHome
