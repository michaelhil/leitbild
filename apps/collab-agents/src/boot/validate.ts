// ============================================================================
// validateBootstrap — single runtime contract check for the wired CollabAgentsWorkspaceRuntime.
//
// Called once per CollabAgentsWorkspaceRuntime construction (post bootstrap, pre-serve). Throws
...
export const validateBootstrap = (system: CollabAgentsWorkspaceRuntime, ctx: ValidateBootstrapContext = {}): void => {
  // === Provider stack contracts ===

  // Contract 1: providerKeys must be wired into the CollabAgentsWorkspaceRuntime.
  ...
  if (!system.providerKeys) {
    fail('providerKeys is missing on CollabAgentsWorkspaceRuntime', 'commit d0c1f73')
  }
  ...
  if (!system.llm) {
    fail('CollabAgentsWorkspaceRuntime.llm (ProviderRouter) is missing', 'arch invariant')
  }
