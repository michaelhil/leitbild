...
export const resolveRoom = (system: CollabAgentsWorkspaceRuntime, roomName: string): Room => {
  const room = system.rooms.getRoom(roomName)
  if (!room) throw new Error(`Room "${roomName}" not found`)
  return room
}

export const resolveAgent = (system: CollabAgentsWorkspaceRuntime, agentName: string): Agent => {
