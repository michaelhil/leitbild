...
- **Relevance to leitbild**: Agents could create sub-rooms with sub-flows for complex tasks
...
- **Relevance to leitbild**: Rooms already function as shared workspaces. Adding structured artifacts (todos, plans) makes them blackboards.

### 6. Multi-Agent Collab Patterns
...
- **Source**: https://www.ibm.com/think/topics/multi-agent-collab
...
- **Relevance to leitbild**: DMs = private memory, rooms = shared memory. Already have the infrastructure.
...
## Leitbild's Unique Position

Leitbild already has infrastructure that maps directly to collaborative planning:

| Leitbild Feature | Planning Analog |
|----------------|-----------------|
...
This is the LLM Blackboard Architecture applied to leitbild, with flow/muting/addressing controls layering on top.
...
Leitbild could have: **living plans that agents negotiate, evolve, and self-organize around**.
...
- Multi-Agent Collab (IBM) — https://www.ibm.com/think/topics/multi-agent-collab
