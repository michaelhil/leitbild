...
## Paper Outline — Research Directions and Design Insights from Leitbild
...
- We describe practical implementations and their trade-offs, based on building Leitbild — a real multi-agent room communication system
...
- In Leitbild, agents respond in plain text with a special prefix `::PASS:: reason` to explicitly decline
...
- **Implemented in Leitbild**: room.post() contains all delivery logic, mode dispatch, staleness calculation, flow advancement
- **Advantages**:
...
- **Leitbild differs**: real-time room-based communication with dynamic mode switching, human participation, and the coordination mechanism as a first-class configurable component
...
### 10.3 Positioning
- We position Leitbild's contribution as: **the coordination mechanism should be a configurable, observable, and potentially AI-driven component** — not a fixed property of the system
