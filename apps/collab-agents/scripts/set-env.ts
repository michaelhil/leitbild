...
// /etc/leitbild/env file and restart the leitbild service.
//
// Usage:
...
//   2. reads /etc/leitbild/env, updates one key, writes it back via stdin
//   3. systemctl restart leitbild
...
const ENV_PATH = '/etc/leitbild/env'
...
console.log('[set-env] restarting leitbild service…')
try {
  await $`ssh -p ${PORT} ${target} systemctl restart leitbild`
} catch (err) {
...
try {
  const status = (await $`ssh -p ${PORT} ${target} systemctl is-active leitbild`.text()).trim()
  if (status === 'active') {
    console.log('[set-env] ✓ leitbild active; new env applied')
  } else {
    console.error(`[set-env] ⚠ leitbild status after restart: ${status}`)
    process.exit(1)
  }
} catch (err) {
