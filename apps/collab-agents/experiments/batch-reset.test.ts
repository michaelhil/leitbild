...
//
// These spawn real leitbild subprocesses — slow (~8s for 4 runs, ~30s+ for
...
describe('runBatch — reset mode (SOAK=1)', () => {
  maybeTest('spawns exactly one subprocess for a 4-run batch; agent/room name re-use works', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'leitbild-batch-reset-'))
    try {
  ...
  maybeTest('50-run soak — no degradation', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'leitbild-batch-soak-'))
    try {
