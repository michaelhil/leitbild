...
**Usage:** Use to set up a new workspace for a project, topic, or collab. The calling agent is added automatically. Choose a clear, unique name. Provide a `roomPrompt` to define the room's purpose.
...
Memory tools provide per-agent persistent storage across sessions. Notes are stored in `~/.leitbild/memory/<agent-name>/`. Load with the filesystem loader by placing the file in `./tools/` or `~/.leitbild/tools/`.
...
## Workspace Capability Tools

Agents interact with other Workspace Modules through the generic Resource and Capability tools. Capability grants identify allowed operations; Resource identities are discovered and selected at invocation time rather than persisted in Agent profiles.

## Adding External Tools

Drop a `.ts` file in `./tools/` (project-local) or `~/.leitbild/tools/` (user-global). The file should export a `Tool` or `Tool[]` as its default export:
...
- Set `LEITBILD_TOOLS_DIR` environment variable to load from a custom path
