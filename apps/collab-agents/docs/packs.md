...
1. They let you equip Leitbild with domain knowledge (air traffic control, driving, research, whatever) without authoring each skill and tool by hand.
...
- **Repo name:** `leitbild-pack-<X>` (e.g. `leitbild-pack-vatsim`).
- **`pack.json` `name` field:** `<X>` (e.g. `"name": "vatsim"`).
- **Install namespace:** `<X>`. Tools register as `<X>_<tool>`, skills as `<X>/<skill>`.

The single source of truth for the install namespace is **`pack.json`'s `name` field**. If `pack.json` is missing or has no `name`, the install falls back to the repo basename with `leitbild-pack-` stripped. The install flow clones to a temp dir, reads the manifest, and only then renames into the final `<packsDir>/<X>` location — so what's on disk always matches what's registered.
...
```env
# Comma-separated. Each entry is either an owner (lists owner/leitbild-pack-*
# repos) or a specific owner/repo.
LEITBILD_PACK_SOURCES=leitbild-packs,michaelhil/leitbild-pack-vatsim
```

Default: `leitbild-packs` — the canonical org. Set `LEITBILD_GH_TOKEN` to lift the unauthenticated rate limit (60 req/hr → 5000 req/hr).
...
```
leitbild-pack-vatsim/         ← repo name (registry discovery)
...
| External tool     | `<name>`           | `~/.leitbild/tools/my_thing.ts` → `my_thing`. Untouched by this feature.         |
...
Identical to authoring any Leitbild tool, with one gotcha: **write the tool's `name` without the prefix**. The loader applies the namespace at registration time:
...
1. Create a new GitHub repo under your account (or under `github.com/leitbild-packs` for the short-name install experience).
2. Put the pack layout at the repo root.
3. `git push`. That's it — no build, no release, no registry.

Users install via `install_pack <name>` (if published under `leitbild-packs`) or `install_pack <user>/<repo>`.

## Management operations

| Tool / Endpoint                         | What it does                                                                    |
| --------------------------------------- | ------------------------------------------------------------------------------- |
| `install_pack` / `POST /api/packs/install` | Clone into `~/.leitbild/packs/<ns>/`, register tools + skills, refresh agents. |
...
Packs are arbitrary TypeScript. Installing one is equivalent to running its code — there is no sandbox. Only install packs from sources you trust. This matches the trust model of `~/.leitbild/tools/` and `~/.leitbild/skills/`: the filesystem itself is the permission boundary.
