...
- **Leitbild integration is discovery-driven.** `src/integrations/leitbild/` never hardcodes Leitbild paths — every endpoint resolves via the deployment's `/.well-known/leitbild` manifest's `links` block. Required link rels are validated on first fetch; missing any fails loud. Mirror service handles `controlInstance.reset` events AND seq regression as epoch boundaries (re-anchors via snapshot). Bound-agent message filter drops `cause.kind === 'external-mirror'` from LLM context EXCEPT `:reset-boundary` (those must pass through). Commands carry stable `actor:samsinn:<slug>` + `client:samsinn:<slug>` identity. `lb_reset` and `lb_clock_set` are intentionally NOT exposed as agent tools — reset/clock changes invalidate reasoning context.
