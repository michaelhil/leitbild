...
- **Cross-Module interaction is generic.** Agents discover Resources and Capabilities through the Workspace Host and invoke them through `workspace_*` tools. Tool Grants name Capabilities only; never persist external Resource ids, base URLs, or application-specific bindings.
- **Module state is strict and independently owned.** Collaboration and Agents have separate markers and snapshots under `workspaces/{workspaceId}/{moduleId}`. Obsolete shapes fail; do not add migrations, compatibility parsers, or cross-owned fields.
