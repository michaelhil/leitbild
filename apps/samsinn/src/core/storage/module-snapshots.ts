...
import type { SamsinnModuleId } from '../workspaces/module-state.ts'
...
export interface WorkspaceModuleSnapshots {
  readonly collaboration: CollaborationSnapshot | null
...
const collaborationSnapshotSchema = z.object({
...
export const serializeModuleSnapshots = (runtime: SerializableRuntime): {
  ...
  return {
    collaboration: collaborationSnapshotSchema.parse({
...
const collaborationIsEmpty = (snapshot: CollaborationSnapshot): boolean =>
...
export const saveWorkspaceModuleSnapshots = async (
  snapshots: { readonly collaboration: CollaborationSnapshot; readonly agents: AgentsSnapshot },
  paths: WorkspaceModulePaths,
  enabledModules: ReadonlySet<SamsinnModuleId>,
): Promise<void> => {
  if (enabledModules.has('collaboration')) {
    if (collaborationIsEmpty(snapshots.collaboration)) await removeSnapshot(paths.collaboration.snapshot)
    else await writeSnapshot(collaborationSnapshotSchema.parse(snapshots.collaboration), paths.collaboration.snapshot)
  }
...
export const loadWorkspaceModuleSnapshots = async (
  ...
  collaboration: enabledModules.has('collaboration')
    ? await loadStrict(paths.collaboration.snapshot, collaborationSnapshotSchema)
    : null,
...
export const restoreWorkspaceModuleSnapshots = async (
  ...
  const collaboration = snapshots.collaboration
  if (collaboration) {
    const scrubbed = new Set((collaboration.pendingScrubs ?? []).map(scrub => scrub.packId))
    for (const roomSnapshot of collaboration.rooms) {
  ...
  if (collaboration) {
    if (runtime.spawnHumanAgent) {
      for (const human of collaboration.humanActors) {
    ...
    runtime.bookmarks.restore(collaboration.bookmarks)
  }
}

export const appendCollaborationPendingScrub = (
  path: string,
  scrub: PendingScrub,
): Promise<{ readonly applied: boolean; readonly reason?: string }> => writeChain.run(async () => {
  const snapshot = await loadStrict(path, collaborationSnapshotSchema)
  if (!snapshot) return { applied: false, reason: 'no Collaboration snapshot' }
  if ((snapshot.pendingScrubs ?? []).some(item => item.packId === scrub.packId)) {
    return { applied: false, reason: 'already queued' }
  }
  const next = collaborationSnapshotSchema.parse({
    ...snapshot,
