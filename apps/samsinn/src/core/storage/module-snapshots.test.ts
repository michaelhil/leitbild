...
import {
  appendCollaborationPendingScrub,
...
describe('Workspace Module snapshots', () => {
  test('separates Collaboration state from Agent Profiles', () => {
    ...
    expect(snapshots.collaboration.rooms).toHaveLength(1)
    expect(snapshots.collaboration.bookmarks).toHaveLength(1)
    expect(snapshots.collaboration).not.toHaveProperty('agents')
  ...
  test('persists, loads, and removes Module state independently', async () => {
    ...
    const priorHome = process.env.SAMSINN_HOME
    process.env.SAMSINN_HOME = temporaryRoot
    try {
      ...
      const snapshots = serializeModuleSnapshots(source)
      await saveWorkspaceModuleSnapshots(snapshots, paths, new Set(['collaboration', 'agents']))

      const both = await loadWorkspaceModuleSnapshots(paths, new Set(['collaboration', 'agents']))
      expect(both.collaboration?.rooms).toHaveLength(1)
      expect(both.agents?.agents).toHaveLength(1)

      await rm(paths.agents.root, { recursive: true, force: true })
      const collaborationOnly = await loadWorkspaceModuleSnapshots(paths, new Set(['collaboration']))
      expect(collaborationOnly.collaboration?.rooms).toHaveLength(1)
      expect(collaborationOnly.agents).toBeNull()
    } finally {
      if (priorHome === undefined) delete process.env.SAMSINN_HOME
      else process.env.SAMSINN_HOME = priorHome
    }
  })
  ...
  test('rejects obsolete or cross-owned fields instead of migrating them', async () => {
    ...
    try {
      ...
      await Bun.write(paths.collaboration.snapshot, JSON.stringify({
        schemaVersion: 29,
      ...
      expect(loadWorkspaceModuleSnapshots(paths, new Set(['collaboration']))).rejects.toThrow()
    } finally {
      if (priorHome === undefined) delete process.env.SAMSINN_HOME
      else process.env.SAMSINN_HOME = priorHome
    }
  ...
  test('restores Agent Room membership from Collaboration ownership', async () => {
    const source = runtime()
    const room = source.rooms.createRoom({ name: 'Operations', createdBy: 'human-1' })
    room.restoreState({
      members: ['agent-1'],
  ...
  test('queues Pack scrubs only in Collaboration state', async () => {
    ...
    try {
      ...
      await saveWorkspaceModuleSnapshots(serializeModuleSnapshots(source), paths, new Set(['collaboration']))
      expect((await appendCollaborationPendingScrub(paths.collaboration.snapshot, {
        packId: 'aviation',
        scheduledAt: '2026-08-29T18:00:00.000Z',
      })).applied).toBe(true)
      const loaded = await loadWorkspaceModuleSnapshots(paths, new Set(['collaboration']))
      expect(loaded.collaboration?.pendingScrubs?.[0]?.packId).toBe('aviation')
    } finally {
      if (priorHome === undefined) delete process.env.SAMSINN_HOME
      else process.env.SAMSINN_HOME = priorHome
    }
