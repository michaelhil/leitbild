...
describe('canonical application API paths', () => {
  test('resolves a Workspace path to an internal application path', () => {
    expect(resolveApplicationApiPath(`/api/workspaces/${workspaceId}/rooms`)).toEqual({
      kind: 'workspace',
      workspaceId,
      internalPath: '/rooms',
    })
  ...
  test('resolves the Workspace realtime endpoint', () => {
    expect(resolveApplicationApiPath(`/api/workspaces/${workspaceId}/ws`)).toEqual({
      kind: 'workspace',
      workspaceId,
      internalPath: '/ws',
    })
  ...
  test('rejects malformed Workspace ids', () => {
    expect(resolveApplicationApiPath('/api/workspaces/not-a-uuid/rooms')).toEqual({
      kind: 'invalid-api',
