...
import type { SamsinnModuleState } from '../core/workspaces/module-state.ts'
...
import { handleSamsinnModuleApi } from './workspace-module-api.ts'
...
const workspaceIdFromPagePath = (pathname: string): WorkspaceId | null => {
  const match = pathname.match(/^\/workspaces\/([^/]+)(?:\/samsinn)?$/)
...
const serveStatic = async (pathname: string, uiPath: string, transpiler: Bun.Transpiler): Promise<Response | null> => {
  if (workspaceIdFromPagePath(pathname) !== null || pathname === '/index.html') {
    ...
    return new Response('<h1>Samsinn</h1><p>UI unavailable.</p>', { headers: { 'Content-Type': 'text/html' } })
  }
...
export const createServer = (config: ServerConfig) => {
  ...
  const server = Bun.serve<WSData>({
    ...
    async fetch(request, bunServer) {
      ...
      const moduleResponse = await handleSamsinnModuleApi(request, url, {
      ...
      if (staticResponse) {
        if (pageWorkspaceId !== null && !(await registry.exists(pageWorkspaceId))) {
          return secure(Response.json({ error: { code: 'workspace_not_provisioned', message: 'Samsinn is not enabled in this Workspace' } }, { status: 404 }))
        }
  ...
  console.log(`Samsinn Modules listening on http://${bindHost}:${port}`)
  return server
}
