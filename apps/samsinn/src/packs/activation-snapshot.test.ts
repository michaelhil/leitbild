...
describe('evicted Collaboration Pack scrub round-trip', () => {
  let temporaryRoot = ''
  ...
  test('drains queued Pack removals without touching Agent state', async () => {
    priorHome = process.env.SAMSINN_HOME
    temporaryRoot = await mkdtemp(join(tmpdir(), 'collaboration-scrub-'))
    ...
    const queued = await appendCollaborationPendingScrub(paths.collaboration.snapshot, {
      packId: 'aviation',
      scheduledAt: '2026-08-29T18:00:00.000Z',
    })
    expect(queued.applied).toBe(true)

    const snapshots = await loadWorkspaceModuleSnapshots(paths, new Set(['collaboration']))
    ...
    expect(next.collaboration.pendingScrubs).toBeUndefined()
  })
})
