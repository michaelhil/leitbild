...
- **Multiple sandboxes.** Settings → Instances lets you list, create, switch, and delete isolated session instances. The current cookie binds you to one; share a link with `?join=<id>` to hand someone else access. See the README "multi-Workspace" section and [`deploy/RUNBOOK.md`](../deploy/RUNBOOK.md).
