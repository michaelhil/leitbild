...
- **`main.ts` at 957 LOC** (under the 1100 LOC threshold that triggers a refactor). Stable in current form; the lateBinding-slot pattern is documented as a rejected refactor in CLAUDE.md. **Trigger to revisit:** main.ts crosses 1100 LOC, OR a self-contained subsystem (like `ollama-urls.ts` was) becomes extractable.
