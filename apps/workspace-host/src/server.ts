      ...
      try {
        ...
        if (url.pathname === '/' && request.method === 'GET') {
          ...
          if (workspaces.length === 0) {
            await config.host.create({ name: null, experienceIds: [...(config.initialExperienceIds ?? [])] })
            workspaces = config.host.list()
          }
          if (workspaces.length === 1) {
            const workspace = workspaces[0]!
            const initialExperienceId = config.initialExperienceIds?.length === 1
              ? config.initialExperienceIds[0]
              : undefined
            if (initialExperienceId !== undefined) {
              const experience = config.host.experiences(workspace.id)
        ...
        if (experienceEntryMatch && request.method === 'GET') {
          const workspaceId = workspaceIdSchema.parse(decodeURIComponent(experienceEntryMatch[1] ?? ''))
          const experienceId = experienceIdSchema.parse(decodeURIComponent(experienceEntryMatch[2] ?? ''))
          return Response.redirect(await config.host.experienceEntryUrl(workspaceId, experienceId), 303)
        }
