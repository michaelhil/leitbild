...
import { pathDataFor, pathPointsFor } from '../src/ui/process-display/process-display-rendering.ts'

const pressurizedWaterReactorPlantSpec = assemblePwrReferencePlantGraph({ loopCount: 4 })
const referenceDisplay = () => compileProcessDisplay({
...
describe('process display rendering helpers', () => {
  test('preserves authored route points while following moved endpoints', () => {
    const display = referenceDisplay()
    const path = display.paths.find(candidate => candidate.id === 'main-steam-to-turbine')
