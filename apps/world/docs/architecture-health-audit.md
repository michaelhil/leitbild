...
Later PWR credibility work moved the acceptance and benchmark harnesses onto the modular PWR assembly path and expanded acceptance to `186/186` checks. The current assembled-graph benchmark measures roughly `323x` realtime for one four-loop system and `78x` realtime for six independent four-loop systems on the same local hardware. The source-backed PWR credibility suite now runs graph-aware reference I&C and currently has no gate or watch failures across its active operational target set.
