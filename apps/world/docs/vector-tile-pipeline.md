...
`maps:terrain:ingest` clones the active vector release into a new release directory, validates the DEM PMTiles header, writes `terrain.pmtiles` and `terrain.json`, and leaves promotion explicit. `maps:status` reports whether the active `current/terrain.pmtiles` is valid. `/map/capabilities.json` marks terrain `available` only after the promoted archive validates as readable PNG DEM PMTiles data; a corrupt or unreadable file remains explicit `unavailable` context rather than fake elevation.
...
Terrain is a separate optional map-context product, not baked into the OSM vector base map. The current contract expects a Terrarium or Mapbox-encoded PNG PMTiles DEM at `/opt/leitbild/maps/current/terrain.pmtiles`. The capability manifest advertises the terrain tileset even when the file is absent or unreadable, with `availability.status: "unavailable"`, so Babylon views and diagnostics can fail visibly instead of pretending flat procedural scenery has real elevation. Preferred source for Norway is Kartverket DTM; Copernicus DEM GLO-30 is the global fallback candidate for areas outside first-party coverage.

Drone and 3D scenery consumers use the same source-backed layer set. They should not invent city detail when a tile is sparse. Current Babylon scenery consumes building footprints, landcover/landuse, water, waterways, transportation, transportation names, aeroway, POIs, and place labels. Rich visual detail is allowed only as a deterministic transform of those source features: facades and rooftop fixtures from building footprints, road furniture and barriers from road geometry, road-label signs from `transportation_name`, shoreline cues from water polygons, and vegetation from landcover/landuse. Runtime consumers may conservatively merge source-identical or named-major transport fragments to reduce tile-cut artifacts, and derived detail must respect source-backed solid and transport exclusions. If a flight area lacks detail, the fix is a richer promoted vector/terrain artifact or an explicit reference dataset, not procedural replacement of missing ground truth.
...
```sh
curl -fsS https://leitbild.leitbild.app/map/capabilities.json
curl -fsS https://leitbild.leitbild.app/map/style.json
curl -fsSI https://leitbild.leitbild.app/map/tiles/current.pmtiles
curl -fsSI https://leitbild.leitbild.app/map/fonts/Noto%20Sans%20Regular/0-255.pbf
```
