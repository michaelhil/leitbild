...
The registry is the shared language for process surfaces, AI agents, tests, trends, Scenario Timelines, and Pack queries.
...
The current implementation covers graph/spec validation, generic catalog contribution discovery, modular graph assembly, a headless fixed-step runtime and testbed, pack runtime lifecycle integration, runtime-private snapshot/restore, query routing, signal resolution/read/search, external condition evaluation, procedure tag compatibility validation, persistent I&C lifecycle state, validated I&C writes, command gates, writable-variable command paths, compiled process surface reads/snapshots/projections, and read-only credibility evidence list/read queries.

Credibility evidence queries are for browseable validation artifacts, not runtime truth. `process-plant.credibility.list` returns evidence sets registered for one compiled process system, and `process-plant.credibility.read` returns a named artifact such as a generated JSON target summary or SVG report. Evidence is contributed through the process-plant catalog and matched against compiled graph identity/topology. The current PWR reference-family evidence is generated by `bun run process-plant:credibility` and is exposed through the same pack query boundary used by graph artifacts and the process-plant catalog.

Process-plant runtime config owns process-specific mechanics and telemetry sampling. Timed demonstrations are authored as Scenario Timeline Cues that issue the same validated process-plant commands as operators and API clients; the process-plant Pack owns command validation and execution.
