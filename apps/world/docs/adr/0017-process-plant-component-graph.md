...
The provider owns process mechanics and telemetry buffers. Timed demonstrations use Scenario Timeline Cues to issue validated process-plant commands, while the Pack remains responsible for command meaning, limits, simulation, and trend data. Core therefore schedules generic command invocations without becoming a process-control language.
