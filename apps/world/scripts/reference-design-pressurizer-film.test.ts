...
test('finite film reference declares resolution and duration, not a fitted drain delay', () => {
  const value = { duration_s: 30, axialCells: 40, radialCells: 12, timeStep_s: .1 }
  ...
  expect(() => parseFilmBasis(block({...value, radialCells: 1}))).toThrow()
  expect(() => parseFilmBasis(block({...value, timeStep_s: .3}))).toThrow()
