...
export const joinedCalculation=phaseStorageThermodynamics+filmDefinitions+String.raw`
...
def run(n,nr,dt,nq=4,cold=False,zero=False,bandCount=None):
    ...
    for lo,hi in reversed(list(zip(edges[:-1],edges[1:]))):
        temp=sourceT-(b['phase']['coldLowerOffset_K'] if cold and hi<=Lsource/2 else 0)
    ...
    def pool(ps,sR,mu):
        ...
        for c in material:
            ...
            splits=[bottom]+([cross] if bottom<cross<top else [])+[top];eu=ep=vv=0.;below=V
            ...
            bands.append(dict(M=c['mass'],s=c['entropy'],V=vv,E=eu+ep+g/A*c['mass']*below,
                belowVolume=below,aboveVolume=V,pressureBelow=(ps+g*(total-bottom)/A/1e6)*1e6,pressureAbove=(ps+g*(total-top)/A/1e6)*1e6));bottom=top
    ...
    wallSolve,wallMass,_,_=make_wall(nr,dx)
    ...
    profile=vaporProfile(p0,sv0,L0);ds=np.zeros(n);surfaces=np.array([sat(profile((ze[i]+ze[i+1])/2).P)[0] if zero else Tw0 for i in range(n)],dtype=float)
    ...
    for step in range(steps):
        def candidate(x):
            ps,sv,sR,mu=x;pl=pool(ps,sR,mu);L=pl['L'];dL=L-old['pool']['L']
            ...
            for i in range(n):
                ...
                def interface(surface):
                    ...
                    cond,out,heat,work,stored=terms(d);temperatures,wallQ,res=wallSolve(old['wall'][i],surface,dt,inner_fraction=min(1.,ell/dx))
                    return wallQ[0]*dt-heat,(d,cond,out,heat,work,stored,temperatures,res,ho,hm)
                lower=float(min(old['wall'][i]));upper=ts
                if zero:surface=old['surfaces'][i]
                else:surface=brentq(lambda t:interface(t)[0],lower,upper,xtol=1e-10)
                mismatch,(d,cond,out,heat,work,stored,temperatures,res,ho,hm)=interface(surface)
            ...
            ep=pl['E']-old['pool']['E']-drainE-sweepE-Wp
            ...
            for a,c in zip(old['pool']['bands'][:-1],pl['bands'][:-1]):
                ...
                adiabatic.append(c['E']-a['E']-work)
            return np.array([em,receipt,ep/1e5,ev/1e5]),dict(ps=ps,sv=sv,sR=sR,mu=mu,pool=pl,vapor=vf,film=ff,ds=newfilm,surfaces=newsurfaces,wall=newwall,profile=profile,
        ...
        if not all(math.isfinite(v) for v in [*residual,energy,peakRe,new['localResidual'],new['volumeResidual'],*new['adiabaticWorkResiduals']]):raise ValueError('Nonfinite joined physical residual')
        ...
        rows.append(dict(time_s=(step+1)*dt,pressureSurface_MPa=new['ps'],level_m=new['pool']['L'],vaporMass_kg=new['vapor']['M'],vaporSuperheatMin_K=new['vapor']['minimumSuperheat_K'],
            ...
            independentUnheatedBandWorkResiduals_J=new['adiabaticWorkResiduals'],volumeResidual_m3=new['volumeResidual'],localFilmWallResidual_J=new['localResidual']))
        old=new
    preserved=all(a['M']==c['M'] and a['s']==c['s'] for a,c in zip(initialPool['bands'][:-1],old['pool']['bands'][:-1]))
    if not preserved:raise ValueError('Unheated lower material history reset')
    ...
    return dict(axialCells=n,radialCells=nr,timeStep_s=dt,quadrature=nq,coldHistory=cold,zeroDrive=zero,endpointFrozenPropertyRelativeDifferences=propertyDifferences,
        initial=dict(pool=initialPool,vapor=v0,totalMass_kg=totalMass,totalEnergy_J=E0),rows=rows,unheatedMaterialHistoryPreserved=preserved,zeroCondensationDriveHold=zeroHold,maximumEnergyResidual_J=maximumEnergy,maximumMassResidual_kg=maximumMass)
def crossing_check():
