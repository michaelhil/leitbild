...
try {
  ...
  const places = [
    ...countries.flatMap(feature => typeof feature.properties.LABEL_X === 'number' && typeof feature.properties.LABEL_Y === 'number' ? [{ type: 'Feature', geometry: { type: 'Point', coordinates: [feature.properties.LABEL_X, feature.properties.LABEL_Y] }, properties: { name: feature.properties.NAME_EN ?? feature.properties.NAME, kind: 'country' }, tippecanoe: { minzoom: 0, maxzoom: 4 } }] : []),
    ...cities.map(feature => ({ type: 'Feature', geometry: feature.geometry, properties: { name: feature.properties.name, kind: 'city' }, tippecanoe: { minzoom: Number(feature.properties.scalerank) <= 3 ? 2 : 4, maxzoom: 6 } })),
  ]
