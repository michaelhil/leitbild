<script lang="ts">
  ...
  let { cues, commands, onchange, validate }: { cues: Cue[]; commands: ScenarioAuthoringCatalog['commands']; onchange: (cues: Cue[]) => void; validate: (cues: Cue[]) => Promise<void> } = $props()
...
<section>
  ...
  {#each cues as cue, index (cue.id)}
    <details>
      ...
      {#each cue.actions as action, actionIndex}
        {#if action.type === 'invoke_capability'}
          ...
          <AdvancedConfiguration label="Command input" value={action.input} onapply={async input => {
            ...
            await validate(cues.map((value, i) => i === index ? updated : value)); replace(index, updated)
          }} />
      ...
      <AdvancedConfiguration label="Cue configuration" value={cue} onapply={async value => { const next = cues.map((item, i) => i === index ? value as Cue : item); await validate(next); onchange(next) }} />
