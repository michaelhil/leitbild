...
import { simulationRunEventSchema, type SimulationRunEvent } from '../model/index.ts'

export interface EventLog {
  readonly appendMany: (events: ReadonlyArray<SimulationRunEvent>) => Promise<void>
  readonly readAll: () => Promise<ReadonlyArray<SimulationRunEvent>>
  readonly readAfter: (seq: number) => Promise<ReadonlyArray<SimulationRunEvent>>
  readonly readLast: () => Promise<SimulationRunEvent | null>
  readonly readLastSeq: () => Promise<number>
  readonly sizeBytes: () => Promise<number>
}

const assertStrictSequence = (events: ReadonlyArray<SimulationRunEvent>, context: string, previousSeq = -1): number => {
...
const readEvents = async (path: string): Promise<ReadonlyArray<SimulationRunEvent>> => {
  let text: string
  try {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    const parsed = simulationRunEventSchema.safeParse(raw)
    if (!parsed.success) {
    ...
    events.push(parsed.data as SimulationRunEvent)
  }
  return events
}

const parseEventLine = (line: string, location: string): SimulationRunEvent => {
  let raw: unknown
  try {
    raw = JSON.parse(line) as unknown
  } catch (err) {
  ...
  return parsed.data as SimulationRunEvent
}
...
const readLastEvent = async (path: string): Promise<SimulationRunEvent | null> => {
...
export const createJsonlEventLog = (path: string): EventLog => {
  ...
  const readAll = async (): Promise<ReadonlyArray<SimulationRunEvent>> => readEvents(path)
  const readLast = async (): Promise<SimulationRunEvent | null> => readLastEvent(path)
  ...
  return {
    appendMany: async (events: ReadonlyArray<SimulationRunEvent>): Promise<void> => {
    ...
    readAll,
    readAfter: async (seq: number): Promise<ReadonlyArray<SimulationRunEvent>> =>
