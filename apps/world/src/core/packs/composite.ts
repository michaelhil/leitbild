...
const packId = (pack: WorldPack): string => pack.descriptor.id

const packForObject = (packs: ReadonlyArray<WorldPack>, object: OperationalObject): WorldPack | null => {
...
const packForCreateType = (packs: ReadonlyArray<WorldPack>, typeId: string): WorldPack => {
...
): ReadonlyArray<WorldPack> =>
  packs.filter(pack => pack.commands.isController(controller) && pack.commands.isTarget(controller, target, context))

const packForCancelCommand = (packs: ReadonlyArray<WorldPack>, controller: OperationalObject): WorldPack => {
...
export const createCompositePack = (config: {
  ...
  const mapAreaFeatureLayers = (() => {
    const seen = new Set<string>()
    const output: NonNullable<WorldPack['presentation']['mapAreaFeatureLayers']>[number][] = []
  ...
  const mapLayerGroups = (() => {
    const seen = new Set<string>()
    const output: NonNullable<WorldPack['presentation']['mapLayerGroups']>[number][] = []
    for (const pack of config.packs) {
