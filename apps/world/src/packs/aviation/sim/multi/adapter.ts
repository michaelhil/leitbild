...
const readInitialSource = (
  ...
  configuredDefault: AviationSourceId,
): AviationSourceId => {
...
export const createAviationMultiPackRuntimeAdapter = (
  ...
  return {
    ...
    connect: async (connectionConfig: PackRuntimeConnectionConfig): Promise<PackRuntimeConnection> => {
      ...
      if (!sourceForAdapter(config, requestedInitialSource)) {
        throw new Error(`aviation.multi: configured source is not registered: ${requestedInitialSource}`)
      }
      let currentSource: AviationSourceId = requestedInitialSource
      ...
      return {
        ...
        invokeQuery: async (request: PackRuntimeQuery): Promise<unknown> => {
          if (!activeConnection) throw new Error('aviation.multi has no active sub-connection')
          // Pass through to the active source's query handler, then enrich
          // aviation.source_status with the multi's view (active source +
          // tracked count).
          const result = await activeConnection.invokeQuery(request)
