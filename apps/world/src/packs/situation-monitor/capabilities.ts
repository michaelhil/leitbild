...
export const situationCapabilities: ReadonlyArray<SimulationCapability> = [
  ...
  query('record.inspect', 'Inspect external evidence', 'Read one exact record by source and record ID, including provenance, measurements and media references. Does not claim to watch linked video.', z.object({ sourceId: sourceIdSchema, recordId: z.string().max(256) }).strict(), externalRecordSchema),
