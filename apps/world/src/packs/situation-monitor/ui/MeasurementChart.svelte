...
{#if quantities.length}<section aria-label="External measurements"><h3>{forecast ? 'Forecast series' : 'Reported measurements'}</h3><select aria-label="Measurement" bind:value={selected}>{#each quantities as item}<option value={quantityKey(item)}>{item.id} ({item.unit})</option>{/each}</select>
