<script lang="ts">
  ...
  import { invokeSituation, invokeWorld } from './client.ts'
...
<section class="monitor" aria-label="Situation Monitor">
  ...
  <nav><button class:active={tab === 'records'} onclick={() => tab = 'records'}>Records</button><button class:active={tab === 'sources'} onclick={() => tab = 'sources'}>Sources</button><span>{loading ? 'Updating…' : status ? 'Live · real time' : 'Connecting…'}</span></nav>
  <div class="body">
  ...
  {:else}
    ...
    {#if selected}<article class="inspector"><div class="row"><h3>{selected.title}</h3><button aria-label="Close record" onclick={() => selected = null}>×</button></div><p>{selected.summary}</p><dl><dt>Source</dt><dd>{selected.sourceId}</dd><dt>Retrieved</dt><dd>{new Date(selected.retrievedAt).toLocaleString()}</dd>{#if selected.publishedAt}<dt>Published</dt><dd>{new Date(selected.publishedAt).toLocaleString()}</dd>{/if}{#if selected.updatedAt}<dt>Updated</dt><dd>{new Date(selected.updatedAt).toLocaleString()}</dd>{/if}{#if selected.validAt}<dt>Valid at</dt><dd>{new Date(selected.validAt).toLocaleString()}</dd>{/if}{#each selected.measurements as measure}<dt>{measure.id}</dt><dd>{measure.value} {measure.unit}</dd>{/each}</dl><a href={selected.url} target="_blank" rel="noopener noreferrer">Original source ↗</a><p>{selected.attribution}</p>{#if selected.geometry}<button onclick={() => focus(selected!)}>Show on map</button>{/if}{#if selected.media}{#key selected.sourceId + selected.id}<MediaPlayer media={selected.media} />{/key}{/if}<details><summary>All record data</summary><pre>{JSON.stringify(selected, null, 2)}</pre></details></article>{/if}
    {#if selected?.measurements.length}{#key selected.sourceId}<MeasurementChart {simulationRunId} sourceId={selected.sourceId} {dataRevision} />{/key}{/if}
    {#if page?.records.length}{#each page.records as record (record.sourceId + '\u0000' + record.id)}<button class="record" onclick={() => selected = record}><strong>{record.title}</strong><small>{record.kind} · {status?.config.sources.find(source => source.id === record.sourceId)?.name ?? record.sourceId} · {new Date(record.validAt ?? record.publishedAt ?? record.retrievedAt).toLocaleString()}{record.geometry ? ' · Located' : ''}</small><span>{record.summary.slice(0, 180)}</span></button>{/each}{:else if !loading}<p>No retained records match. Open Sources to configure collection or inspect errors. An empty result does not establish that nothing happened.</p>{/if}
