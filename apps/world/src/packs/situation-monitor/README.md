...
- All IDs use the `world.situation-monitor.` prefix and the generic Workspace broker. An Assistant discovers the Pack's operations inside its Room Scope; a Run may explicitly block sensitive operations. Destructive source management is never implied by observation access.
