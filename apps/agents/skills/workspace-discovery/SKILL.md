...
- Search Capability descriptions for the operations that fit the question. Read `authorized` as the Agent's authority, `applicable` as whether the Capability fits the supplied target, and `callable` as both being true. Request exact input schemas only for likely calls; request output schemas only when interpreting the result shape requires them.
