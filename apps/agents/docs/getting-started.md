...
> **Prerequisite.** Leitbild is installed and running, and at least one LLM provider is configured. If the **⛓ Providers** button in the top-left shows a red dot, open it and add either an Ollama URL or a cloud API key (Anthropic, OpenAI, Groq, Gemini, …) before continuing.

---

## The 60-second tour

Leitbild has three regions and you'll spend 95% of your time in the middle one:
...
**Why this mission:** it's the whole premise of Leitbild in five minutes — multiple agents sharing a room, each with their own perspective, with you steering.
...
**Why this mission:** one-off replies are fun; *durable state* is what makes Leitbild useful. Every room has a todo list agents can see and modify.
...
| ▸ Tools | Every tool loaded at startup (built-in + `./tools/` + `~/.leitbild/tools/`) |
...
- **Headless / MCP.** Run Leitbild as an MCP server so another LLM can orchestrate everything through 23 tools: `bun run headless`. See the README section on MCP integration.
- **REST + WebSocket API.** Everything the UI does is available as HTTP + WS — see the README for endpoints.
- **Multiple Workspaces.** Use the root-level **Workspaces** link to create, rename, open, compose, or delete Workspaces. Every Leitbild URL carries its Workspace id, so several Workspaces can stay open in different tabs and the same URL can be shared. See the README and [`deploy/RUNBOOK.md`](../deploy/RUNBOOK.md).
