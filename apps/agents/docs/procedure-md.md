...
Procmd lives **inside existing leitbild wikis** as a convention. A wiki page
...
Other consumers (leitbild UI, future llm-wiki-skills bootstrap, custom
...
- **Procedure parse cache.** In-memory, event-driven invalidation on
  `wiki_changed`.
- **Render integration.** Leitbild UI rendering of `[[wikilinks]]` and
  same-page anchors when displaying a procedure page in chat or panel.
- **`leitbild-handbook` wiki.** A wiki holding the rendered procmd spec
  plus general leitbild introspection content, linked at agent startup.
...
- **Validator distribution.** Reference validator currently lives
  per-wiki; if leitbild ships a procedure executor, the same parser must
  serve both.
