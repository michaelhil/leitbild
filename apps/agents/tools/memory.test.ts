...
const testMemoryDir = join(homedir(), '.leitbild', 'memory', sanitizeName(ctx.callerName))
...
describe('remember + recall + forget', () => {
  test('remember stores a fact and recall retrieves it', async () => {
    ...
    const remResult = await remember!.execute({ key: 'project_name', value: 'Leitbild' }, ctx)
    ...
    expect(recData.value).toBe('Leitbild')
  })
