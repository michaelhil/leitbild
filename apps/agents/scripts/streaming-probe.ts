...
//
// Usage:
//   set -a; source /etc/leitbild/env; set +a
//   bun run scripts/streaming-probe.ts --url https://leitbild.app
//   bun run scripts/streaming-probe.ts --url https://leitbild.app --evict-reload
//
// Exit codes:
...
const main = async (): Promise<void> => {
  const ctx = await bootstrapProbe({
    baseUrl,
    target: 'fresh',
    token: process.env.LEITBILD_TOKEN,
  })
