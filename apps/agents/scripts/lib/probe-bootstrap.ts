...
//   1. Optionally authenticate (when LEITBILD_TOKEN is set in deploy mode).
//   2. Provision both Leitbild Modules for a fresh Workspace id, or select an
...
const SESSION_COOKIE_PREFIX = 'leitbild_session='
...
export const bootstrapProbe = async (opts: BootstrapOptions): Promise<ProbeContext> => {
  ...
  if (target === 'fresh') {
    ...
    if (!provision.ok) fail(`probe Agents provisioning ${provision.status}`)
    const cookie = sessionCookie ?? ''
    const warm = await fetch(`${baseUrl}/api/workspaces/${encodeURIComponent(workspaceId)}/agents/rooms`, { headers: { Cookie: cookie } })
