...
import { AUTO_BUDGET_FALLBACK, AUTO_BUDGET_FLOOR, OUTPUT_RESERVE, SAFETY_MARGIN, computeContextBudget } from './budget.ts'
...
describe('computeContextBudget', () => {
  ...
  test('preserves a useful floor for a pathological tool surface', () => {
    const tools = Array.from({ length: 5_000 }, (_, index) => ({ ...tool, name: `read_${index}`, description: 'large '.repeat(100) }))
    expect(computeContextBudget({ contextMax: 32_000, toolDefinitions: tools }, estimate).budget).toBe(AUTO_BUDGET_FLOOR)
  })
})
