...
describe('AgentsWorkspaceRuntime.llm wiring — router events reach late-bound callbacks', () => {
  test('successful chat fires onProviderBound with agent context', async () => {
    const a = makeGateway()
    const b = makeGateway()
    const setup = makeSetup({ a, b }, ['a', 'b'])
    const system = createAgentsWorkspaceRuntime({ providerConfig: baseConfig, providerSetup: setup })
  ...
  test('callSystemLLM path emits events with agentId=null', async () => {
    ...
    const system = createAgentsWorkspaceRuntime({ providerConfig: { ...baseConfig, order: ['a'] }, providerSetup: setup })
