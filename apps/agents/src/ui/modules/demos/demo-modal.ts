...
import { getDemo, type Demo, type DemoAgentSpec, type DemoPrompt } from './catalog.ts'
...
// Biometrics demo uses this for `leitbild-packs/biometrics`. Awaited so
...
const createDemoAgents = async (
  ...
  for (const spec of specs) {
    ...
    try {
      ...
      // Do not depend on websocket event ordering. Follow-on demo actions
      // inspect these stores immediately after this function.
...
export const openDemoModal = async (
  ...
  if (demo.id === 'biometrics') {
    await ensurePackInstalled('biometrics', 'leitbild-packs/biometrics')
  }
