// Biometrics UI extension — declared by leitbild-biometrics pack via
...
// page-level `leitbild:biometric-stop-all` event fires so any active capture
...
export const createBiometricsExtension = (): UIExtension => {
  ...
  return {
    ...
    unmount: async (): Promise<void> => {
      ...
      try {
        document.dispatchEvent(new CustomEvent('leitbild:biometric-stop-all', { detail: { reason: 'extension-unmount' } }))
