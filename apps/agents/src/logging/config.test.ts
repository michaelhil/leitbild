...
describe('parseLogConfigFromEnv', () => {
  test('defaults when unset → disabled + defaults', () => {
    ...
    expect(c.dir).toMatch(/\.leitbild\/logs$/)
    expect(c.sessionId).toMatch(/^session-\d+-[a-f0-9]{8}$/)
    expect(c.kinds).toEqual(['*'])
  })

  test('reads LEITBILD_LOG_ENABLED=1', () => {
    const c = parseLogConfigFromEnv({ LEITBILD_LOG_ENABLED: '1' })
    expect(c.enabled).toBe(true)
  })

  test('any other value → disabled', () => {
    expect(parseLogConfigFromEnv({ LEITBILD_LOG_ENABLED: 'yes' }).enabled).toBe(false)
    expect(parseLogConfigFromEnv({ LEITBILD_LOG_ENABLED: '0' }).enabled).toBe(false)
    expect(parseLogConfigFromEnv({ LEITBILD_LOG_ENABLED: 'true' }).enabled).toBe(false)
  })

  test('LEITBILD_LOG_DIR overrides default', () => {
    const c = parseLogConfigFromEnv({ LEITBILD_LOG_DIR: '/tmp/custom-logs' })
    expect(c.dir).toBe('/tmp/custom-logs')
  })

  test('LEITBILD_SESSION_ID overrides default', () => {
    const c = parseLogConfigFromEnv({ LEITBILD_SESSION_ID: 'study-A-operator-1' })
    expect(c.sessionId).toBe('study-A-operator-1')
  })

  test('LEITBILD_LOG_KINDS comma-splits + trims', () => {
    const c = parseLogConfigFromEnv({ LEITBILD_LOG_KINDS: ' message.* , tool.* ' })
