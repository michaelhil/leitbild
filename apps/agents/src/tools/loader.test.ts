...
describe('loadToolDirectory', () => {
  ...
  test('valid single-tool file is loaded and registered', async () => {
    ...
    const dir = await mkdtemp(join(tmpdir(), 'leitbild-loader-test-'))
    try {
...
describe('loadExternalTools', () => {
  ...
  test('loads from LEITBILD_TOOLS_DIR env var when set', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'leitbild-env-tools-'))
    const original = process.env.LEITBILD_TOOLS_DIR
    try {
      await writeFile(join(dir, 'env-tool.ts'), `export default {
  name: 'loader_env_tool',
  description: 'Loaded via LEITBILD_TOOLS_DIR',
  parameters: {},
  execute: async () => ({ success: true }),
}`)
      ...
      process.env.LEITBILD_TOOLS_DIR = original
      await rm(dir, { recursive: true })
    }
  })
})
