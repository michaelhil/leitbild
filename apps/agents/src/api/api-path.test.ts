...
describe('canonical application API paths', () => {
  test('resolves a Workspace path to an internal application path', () => {
    expect(resolveApplicationApiPath(`/api/workspaces/${workspaceId}/agents/rooms`)).toEqual({
      kind: 'workspace',
      workspaceId,
      moduleId: 'agents',
      internalPath: '/rooms',
  ...
  test('rejects malformed Workspace ids', () => {
    expect(resolveApplicationApiPath('/api/workspaces/not-a-uuid/agents/rooms')).toEqual({
      kind: 'invalid-api',
