// ============================================================================
// Shared-token auth for the deploy mode. Disabled when LEITBILD_TOKEN env is
...
// /api/auth.
//
// Sessions are STATELESS: the cookie value is sha256(LEITBILD_TOKEN). On each
...
//   - Rotating LEITBILD_TOKEN invalidates every cookie at once — the desired
//     revocation path.
...
const SESSION_COOKIE = 'leitbild_session'
...
const requiredToken = (): string | null => {
  const raw = process.env.LEITBILD_TOKEN
...
// is well out of reach. Override with LEITBILD_AUTH_RATE_LIMIT and
// LEITBILD_AUTH_RATE_WINDOW_MS.
//
...
export const getAuthLimiter = (): RateLimiter => {
  if (!authLimiter) {
    authLimiter = createRateLimiter({
      windowMs: Number(process.env.LEITBILD_AUTH_RATE_WINDOW_MS) || 300_000, // 5 min
      max: Number(process.env.LEITBILD_AUTH_RATE_LIMIT) || 20,
    })
  }
  return authLimiter
}
