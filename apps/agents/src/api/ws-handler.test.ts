...
const dispatch = (ws: FakeWS, session: ClientSession, system: AgentsWorkspaceRuntime, wsManager: WSManager, payload: unknown) =>
...
describe('WSManager.safeSend backpressure', () => {
  ...
  test('sweepStaleSessions drops stale viewer sessions without removing agents', async () => {
    ...
    const fakeSystem = { removeAgent: (id: string) => { removed.push(id); return true } } as unknown as AgentsWorkspaceRuntime
    const wsManager = createWSManager({
