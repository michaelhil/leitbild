# Leitbild experiment runner

Automated batch execution of prompt-configuration variants against a leitbild subprocess. Designed for controlled comparisons: *same trigger, different models / personas / temperatures / tools / seeds → collect conversation data → analyze elsewhere.*
...
Each variant × repeat spawns its own ephemeral leitbild subprocess, drives it over MCP stdio through the four Phase 1 primitives (`create_agent` with seed, `wait_for_idle`, `export_room`, `LEITBILD_EPHEMERAL`), and writes one JSON per run plus an incrementally-updated `summary.json`.
...
Every field below maps 1:1 to the leitbild `AIAgentConfig`. Required: `name`, `model`, `persona`. Everything else is optional; omitting preserves leitbild's default.
...
|---|---|---|---|
| `'subprocess'` (default) | One leitbild subprocess per run | ~7.5s startup + real work | Bulletproof isolation. Every run gets a clean tool registry, skill store, provider state, and process memory. |
...
**Use `subprocess` when**: you want maximum isolation, or you suspect leitbild's in-memory state could affect results across runs (e.g. running with summary-scheduler enabled, or with tools that mutate process-wide state).
...
LLM cost is determined by your spec: model × temperature × message count × repeats. Leitbild subprocess overhead is addressed by the isolation modes above.

## Interrupt handling

SIGINT (Ctrl-C) stops the batch after the in-flight run's result + summary have been written. No partial writes. Running leitbild subprocess is cleanly torn down. The next run in the plan will NOT start.

## Troubleshooting

**"Leitbild subprocess did not become ready within 30000ms"** — the MCP `initialize` handshake didn't complete in time. Usually means leitbild failed to start. The error message carries the last 20 lines of leitbild stderr — read those. Common causes: a provider with a bad API key timing out during model-list warmup, a snapshot read error (irrelevant in ephemeral mode), or a syntax error in a custom tool/skill that was loaded at startup.
...
- **`examples/hello-world.ts`** — one real Anthropic agent, two temperature variants. Requires `ANTHROPIC_API_KEY` in the environment (or a key stored in `~/.leitbild/providers.json`).
...
- Tool-call traces. Messages in the export carry tokens/provider/model/generationMs, but intermediate tool calls are internal to leitbild's evaluation loop and not persisted on messages.
