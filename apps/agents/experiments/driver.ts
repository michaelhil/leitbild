// ============================================================================
// Leitbild subprocess driver.
//
// Spawns one ephemeral leitbild process per run and opens an MCP stdio client
...
// Leitbild entry point — relative to this file's location, then absolute.
const LEITBILD_ENTRY = resolve(import.meta.dir, '../src/main.ts')

const CONNECT_TIMEOUT_MS = 30_000

export interface LeitbildHandle {
  readonly client: Client
  readonly close: () => Promise<void>
}

// env() builds the subprocess environment. We inherit process.env so provider
// API keys flow through, then force LEITBILD_EPHEMERAL so every run starts
...
const buildEnv = (): Record<string, string> => {
  ...
  out.LEITBILD_EPHEMERAL = '1'
  return out
}

export const startLeitbild = async (): Promise<LeitbildHandle> => {
  const transport = new StdioClientTransport({
    command: 'bun',
    args: ['run', LEITBILD_ENTRY, '--headless'],
    env: buildEnv(),
    stderr: 'pipe',
  })
  ...
  const client = new Client(
    { name: 'leitbild-experiment-runner', version: '0.1.0' },
    { capabilities: {} },
  )

  const connectPromise = client.connect(transport)
  const timeoutPromise = new Promise<never>((_, reject) =>
    setTimeout(() => reject(new Error(`Leitbild subprocess did not become ready within ${CONNECT_TIMEOUT_MS}ms`)), CONNECT_TIMEOUT_MS),
  )
  ...
  try {
    ...
    throw new Error(`Leitbild startup failed: ${msg}\n--- leitbild stderr (tail) ---\n${tail}`)
  }
