...
// Two isolation modes:
//
//   'subprocess' (default) — one leitbild subprocess per run. Strongest
...
import { startLeitbild, type LeitbildHandle } from './driver.ts'
...
export interface RunBatchOptions {
  ...
  // Fires whenever the batch spawns a new leitbild subprocess. In 'subprocess'
    ...
    if (mode === 'subprocess') {
      ...
      for (const variant of spec.variants) {
        for (let runIndex = 0; runIndex < repeats; runIndex++) {
          ...
          let handle: LeitbildHandle | null = null
          try {
            handle = await startLeitbild()
            options.onSubprocessStarted?.()
          } catch (err) {
            await recordResult(variant.name, startupErrorResult(
              spec, variant, runIndex,
              `leitbild subprocess startup failed: ${err instanceof Error ? err.message : String(err)}`,
            ))
            continue
          }
      ...
      try {
        ...
        resetFailure = `leitbild subprocess startup failed: ${err instanceof Error ? err.message : String(err)}`
      }
        ...
        outer: for (const variant of spec.variants) {
          for (let runIndex = 0; runIndex < repeats; runIndex++) {
            ...
            if (resetFailure || !handle) {
              ...
              await recordResult(variant.name, startupErrorResult(
                spec, variant, runIndex,
                `batch aborted: ${resetFailure ?? 'no leitbild handle'}`,
              ))
              continue
            }
