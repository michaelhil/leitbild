...
Webcam-based face / attention / expression tracking for Leitbild.

## Install

```
install_pack leitbild-packs/biometrics
```
...
WS commands, and tool factories) lives in leitbild core and is gated on
...
- Uninstalling the pack while the process is running removes the tools
  ...
  clean uninstall, restart the leitbild server after `uninstall_pack`.
- v1 tracks face only. Pose / hand / gesture modules ship as separate
  packs (`leitbild-pose`, `leitbild-gesture`) when those modules land.
