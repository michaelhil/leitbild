...
<g class="alarm-panel-summary" transform="translate(14 32)">
  <text x="0" y="0">
...
<line class="alarm-panel-divider" x1="258" x2="258" y1="12" y2={geometry.height - 12} />
<line class="alarm-panel-divider" x1={geometry.width - 412} x2={geometry.width - 412} y1="12" y2={geometry.height - 12} />

{#if visibleAlarms.length === 0}
  <text class="alarm-panel-empty" x="286" y="47">
...
{:else}
  <g transform="translate(282 14)">
    {#each visibleAlarms as alarm, index (alarm.id)}
      <g class="alarm-panel-row {alarm.severity} {alarm.kind}" transform="translate(0 {index * 16})">
        <rect x="0" y="-10" width="682" height="14" rx="2" />
        <text class="alarm-panel-row-kind" x="6" y="0">{kindLabel(alarm)}</text>
        <text class="alarm-panel-row-title" x="58" y="0">{shorten(alarm.title, 54)}</text>
        <text class="alarm-panel-row-status" x="594" y="0">{rowStatus(alarm)}</text>
        <text class="alarm-panel-row-time" x="654" y="0" text-anchor="end">{elapsedLabel(alarm.firstActiveElapsedMs ?? alarm.lastActiveElapsedMs)}</text>
      </g>
    {/each}
  </g>
{/if}

<g class="alarm-panel-focus" transform="translate({geometry.width - 388} 18)">
  {#if focusAlarm}
