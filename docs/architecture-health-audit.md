...
Later PWR credibility work moved the acceptance and benchmark harnesses onto the modular PWR assembly path and expanded acceptance to `186/186` checks. The current assembled-graph benchmark measures roughly `440x` realtime for one four-loop system and `76x` realtime for six independent four-loop systems on the same local hardware. The source-backed PWR credibility suite currently has no gate failures and one watch-level all-RCP-trip fuel-heatup calibration item.
