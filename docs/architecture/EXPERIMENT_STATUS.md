...
The orchestration model is Definition → Resource → Capability. World owns Scenario and Fragment compilation; Agents owns Room Definitions, Agent Scripts, and Prompt Decks; the Host owns only apply-once Presets that reference pinned Module Definition Revisions. Ongoing automation stays inside the owning Module rather than a shared workflow engine.
