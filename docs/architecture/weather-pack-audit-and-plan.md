...
- Production smoke testing tightened the demonstration route so the ambulance starts inside the weather area and immediately exercises its configured low-visibility response. Weather detail values are compactly formatted (raw query/recording values remain unchanged); the open inspector refreshes without dropping its last result. Long rail details no longer overflow off the left edge. Removed a misleading wall-clock arrival estimate from Ambulance presentation; remaining travel time is explicitly simulation time.
...
- Run `world.scenario-authoring.describe` and normal Workspace discovery to find supported authoring operations. Agents use `workspace_explore` and `workspace_call`; no Weather-specific Agents demo tool or hardcoded run ID is needed.
