...
### Live-test finding: optional tool selectors at the provider boundary

A controlled two-request diagnostic reproduced optional history selectors becoming populated on the OpenRouter→OpenAI Responses route. The upstream debug body showed the same schema and request, differing only in omitted versus explicit `strict:false`. The former returned every optional selector; the latter returned the requested empty object. Internal normalization is not directly exposed, but this matches [Responses' documented strict-mode default](https://developers.openai.com/api/docs/guides/function-calling#strict-mode). Extra selectors can make both history listing and actual execution retrieval impossible under the tool's deliberately exclusive selector rules.
