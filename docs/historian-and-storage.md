...
There is no Leitbild application backup timer in the inspected systemd inventory. Provider/external backup coverage is **unverified**. A release artifact is not a state backup. The unresolved operator decision is the off-host destination, encryption key custody and acceptable recovery window; code cannot infer those safely.
